...
module.exports = class PageGenerator extends BaseGenerator {
    ...
    protected static readonly ERROR_COURSE_DOESNT_EXISTS = "Please, initialize a environment with 'yo haztivity:environment' before use the page generator";
    protected static readonly ERROR_SCO_DOESNT_EXISTS = "Please, create a SCO with 'yo haztivity:sco' before use the page generator";
    initializing() {
        this.data = this._getData();
        if(this.options.full != true) {
            let dest = this.destinationRoot(),
                directories = this._getDirectories("src/course");
    ...
    writing() {
        this.fs.copyTpl(
            this.templatePath('**'),
            this.destinationPath(this._resolvePageDir(this.data.pageName)),
            this.data
        );
    }
    install(){
