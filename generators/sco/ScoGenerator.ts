...
module.exports = class ScoGenerator extends BaseGenerator {
    protected static readonly ERROR_SCO_EXISTS = "The SCO indicated already exists. Please, choose another name";
    protected static readonly ERROR_COURSE_FOLDER_DOESNT_EXISTS = "Please, initialize a environment with 'yo haztivity:environment' before use the sco generator";
    initializing() {
    ...
    writing() {
        this.fs.copyTpl(
            this.templatePath('**'),
            this.destinationPath(path.resolve(this.destinationRoot(),"src/course",this.data.scoName)),
            this.data
        );
    }
    install(){
