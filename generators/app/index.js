...
module.exports = class extends Generator {
    ...
    initializing() {
        this.log(yosay(`Welcome to ${chalk.red('generator-haztivity')} generator!. I will ask you some questions to generate the project structure. Go ahead!`));
    }
    ...
    prompting() {
        ...
        let prompts = [
            ...
            },
            {
                type: 'input',
                name: 'pageName',
                message: `Nice!, the SCO should have at least one page. How I call the first ${chalk.cyan("page")}?. ${chalk.red("Note:")} only could contains [a-zA-Z0-9_], for example, page1:`,
                validate: (toValidate) => {
                    return this._validateNonEmpty(toValidate) && this._validateSpecial(toValidate);
                }
            },
    ...
    writing() {
        ...
        this.fs.copy(this.templatePath('./src/course/course/assets/**'), this.destinationPath(`./src/course/${this.data.scoName}/assets`));
        this.fs.copyTpl(this.templatePath('./src/course/course/*'), this.destinationPath(`./src/course/${this.data.scoName}`), this.data);
        this.fs.copyTpl(this.templatePath('./src/course/course/pages/page/*'), this.destinationPath(`./src/course/${this.data.scoName}/pages/${this.data.pageName}`), this.data);
    }
    conflicts() {
    }
    install() {
